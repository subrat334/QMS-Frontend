...
import { API } from "../../services/AllApiServices"; // adjust path if needed
...
const Users: React.FC = () => {
  ...
  // All counters (optional, from initial full fetch) - still kept for compatibility
  const [counters, setCounters] = useState<CounterSimple[]>([]);
  // const [ setLoadingCounters] = useState(false);
  ...
  // UI toggle to show/hide counters area like your screenshot
  const [showCountersArea, setShowCountersArea] = useState(true);

  // Form fields for create/edit
  const [name, setName] = useState("");
  const [employeeId, setEmployeeId] = useState("");
  const [designation, setDesignation] = useState<number>(3); // default Counter
  ...
  const [selectedCounterIds, setSelectedCounterIds] = useState<number[]>(
    []
  ); // store counter.Id (backend 'Id')

  // Modes
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [editUserId, setEditUserId] = useState<string | null>(null); // editing privileges for existing user
  ...
  //     // convert to CounterSimple shape
  //     const data = (res.data || []).map((c: any) => ({
  //       Id: Number(c.Id),
  //       Name: c.Name,
  //       CategoryId: Number(c.CategoryId),
  //       CategoryName: c.CategoryName || c.CategoryName || c.CategoryName,
  //       SubCategoryId: Number(c.SubCategoryId),
    ...
    raw.forEach((cat: any) => {
      (cat.SubCategories || []).forEach((sub: any) => {
        (sub.Counters || []).forEach((ctr: any) => {
          flattened.push({
            Id: Number(ctr.CounterId),                 // internal id
            CounterId: Number(ctr.CounterId),          // backend id
            Name: ctr.CounterName,                     // name
  ...
  const toggleSubcategory = async (subId: number) => {
    ...
    if (isCurrentlySelected) {
      ...
      // remove counters from selectedCounterIds that belong to this subcategory
      setSelectedCounterIds((prev) =>
        prev.filter((cid) => {
      ...
      // optionally remove cached counters for that subcat
    ...
    // set loader for this subcat
    setLoadingCountersBySubcat((prev) => ({ ...prev, [subId]: true }));

    try {
      // API call that you asked for:
      // /Counter/GetCounterdetailsbycategoryid?SubCatid=<id>
     const res = await API.getCountersBySubCategoryId(subId);

// ALWAYS keep this line — it defines "received"
...
const normalized = received.map((c: any) => ({
  ...
  // available counters for currently selected subcategories (flattened)
  // const availableCountersForSelected = () => {
  //   const arr: CounterSimple[] = [];
  //   selectedSubcategories.forEach((subId) => {
  //     const cs = subcatCounters[subId] || [];
  //     cs.forEach((c) => arr.push(c));
  //   });
  //   return arr;
  // };
  ...
  const buildPrivilegesPayload = (userId: string) => {
    ...
    const subs = selectedSubcategories.map((subId) => {
      ...
      const countersForSub = selectedCounterIds
        ...
        .map((c) => ({ CounterId: Number(c!.CounterId) })); // backend expects CounterId (numeric string -> number)

      return {
        SubCategoryId: subId,
        Counters: countersForSub,
      };
    });
  ...
  const handleCreateUserAndPrivileges = async () => {
    ...
    try {
      ...
      const createPayload = {
        FirstName: name.trim(),
        UserType: designation, // already number mapping
      ...
      toast.success("User created and privileges assigned");
      resetForm();
      await loadUsers();
    } catch (err: any) {
  ...
  const openEditPrivileges = async (userId: string) => {
    try {
      ...
      // d.UserType may be string like "Counter" — keep select by label mapping or default to Counter
      // keep designation as number by best-effort mapping:
      const ut = String(d.UserType || "").toLowerCase();
      if (ut.includes("mis")) setDesignation(5);
      else if (ut.includes("monitor")) setDesignation(4);
      else if (ut.includes("patient")) setDesignation(6);
      else setDesignation(3);
      ...
      if (firstCat) {
        ...
        (firstCat.SubCategories || []).forEach((s: any) => {
          (s.Counters || []).forEach((c: any) => {
            // backend counter entries have an 'Id' field — we store that to map to counters list
...
const normalized = received.map((c: any) => ({
  ...
  SubCategoryId: sid,
  SubCategoryName:
    (firstCat.SubCategories || []).find((s: any) => Number(s.SubCategoryId) === sid)?.SubCategoryName || "",
}));
  ...
  // There is no password endpoint specified — show UI and indicate not implemented if backend missing.
...
const handleEmployeeIdChange = (value: string) => {
  ...
  // Allow letters, numbers, space, and special chars: - _ @ # / .
  value = value.replace(/[^A-Za-z0-9 @#_\-\/.]/g, "");
 
  // Replace multiple spaces with one
  value = value.replace(/\s+/g, " ");
 
  // Max length 25 (you can change)
  if (value.length > 25) {
    ...
    <div className="p-6 w-full">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {/* <div className="flex gap-2">
          <button
            onClick={async () => {
              await Promise.all([loadUsers(), loadCategories(), loadSubcategories(), loadCounters()]);
              toast.success("Refreshed");
            }}
            className="flex items-center gap-2 bg-white border border-green-200 px-3 py-1 rounded shadow-sm hover:bg-green-50"
          >
...
{selectedCategoryId && (
  ...
  >
    {showCountersArea ? "Hide Counters" : "Show Counters"}
            ...
            {/* Counters for selected subcategories (grouped) */}
            {selectedSubcategories.length > 0 && (
              <div className="mt-6 p-4 rounded-md border border-green-100 bg-green-50 transition-all"
                        ...
                        <div className="flex flex-wrap gap-3">
                          {countersForSub.length === 0 ? (
                            ...
                            countersForSub.map((ctr) => (
                              <label
                                key={ctr.CounterId}
                                className={`px-4 py-2 rounded-full border cursor-pointer select-none transition ${selectedCounterIds.includes(ctr.CounterId)
                                  ? "bg-green-600 text-white border-green-700"
                                  : "bg-white border-green-200 text-green-700 hover:bg-green-50"
                                  }`}
                                onClick={() => {
                                  console.log("COUNTER CLICKED →", ctr.CounterId);
                              ...
                              >
                                {/* <input
                                  type="checkbox"
                                  checked={selectedCounterIds.includes(ctr.CounterId)}
                                  onChange={() => { }}
                                  className="sr-only"
                                /> */}
                                {ctr.Name}
    ...
    ${isSubmitting || !isCreateFormValid
  ...
  `}
                ...
                {/* <button
                onClick={resetForm}
                className="bg-gray-100 border border-gray-200 px-4 py-2 rounded hover:bg-gray-200"
              >
                Cancel / Reset
              </button> */}
            </div>
